...
contract YieldToken is ERC20("Dustz", "DTZ") {
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		...
		if (timerUser > 0)
			rewards[_user] = rewards[_user] + cryptogemsContract.balanceOf(_user) * BASE_RATE * (time - timerUser) / 86400 + _amount * INITIAL_ISSUANCE;
		else 
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		...
		if (_tokenId < 1001) {
			...
			if (timerFrom > 0)
				rewards[_from] += cryptogemsContract.balanceOf(_from) * BASE_RATE * (time - timerFrom) / 86400;
			else
			...
			if (_to != address(0)) {
				...
				if (timerTo > 0)
					rewards[_to] += cryptogemsContract.balanceOf(_to) * BASE_RATE * (time - timerTo) / 86400;
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = cryptogemsContract.balanceOf(_user) * BASE_RATE * (time - lastUpdate[_user]) / 86400;
